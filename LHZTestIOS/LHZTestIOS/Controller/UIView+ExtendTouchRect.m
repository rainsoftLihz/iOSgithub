...
+ (void)load {
    Swizzle(self, @selector(pointInside:withEvent:), @selector(lz_PointInside:withEvent:));
}
...
- (BOOL)lz_PointInside:(CGPoint)point withEvent:(UIEvent *)event {
    if (UIEdgeInsetsEqualToEdgeInsets(self.touchExtendInset, UIEdgeInsetsZero) || self.hidden ||
        ...
        return [self lz_PointInside:point withEvent:event]; // original implementation
    }
